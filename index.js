...
const dataCloud=document.querySelector("[data-clouds]")
    ...
    function getLocation() {
        if (navigator.geolocation) {
            const options = {
                timeout: 10000, // Adjust the timeout in milliseconds (default is 0, which means no timeout)
            };
            navigator.geolocation.getCurrentPosition(showPosition, handleLocationError, options);
        } else {
            alert("Your device/browser does not support geolocation.");
        }
    }
...
//                 // Now you can use these coordinates to fetch weather information
//                 // You can call a function here to proceed with weather data retrieval
//             },
//             error => {
//                 console.error('Error getting location:', error);
//             }
